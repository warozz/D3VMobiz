...
export class TopPlanService {
                    ...
                    if (objMs != null && objMs.length > 0) {
                        ...
                        this.db.create(ConectionDBName.connectionDB).then((db: any) => {
                            ...
                            this.sqlitePorter.importJsonToDb(dbInstance, sqlJsonBlock).then(
                                ...
                                (err) => {
                                    console.log("******************* Insert topplan error " + JSON.stringify(err));
                                    reject(err);
                                }
                            );
                          });
    ...
    public delete(params: any) {
        ...
        return new Promise((resolve, reject) => {
            this.createTable().then(
                (res) => {
                    let objMs: Array<TopPlanM> = params; 
                    let paramSQL = [];
                    ...
                    this.db.create(ConectionDBName.connectionDB).then(
                        (connection: SQLiteObject) => {
                            connection.executeSql(sql, paramSQL).then(
                                ...
                                , (error) => {
                                    console.log("!!!!!!!!!!!!!!!!!!!!!!!! ==> Delete "+ this.TABLENAME +" error." + JSON.stringify(error));
                                    reject(error);
                                }
                            )
                        }
    ...
    public search(params: any) {
        //console.log("----------- TopPlanService : search ------------" + JSON.stringify(params));

        return new Promise((resolve, reject) => {
            this.createTable().then(
                (res) => {
                    let objM: Array<TopPlanM> = params; 
                                ...
                                (result) => {
                                    ...
                                    if (result.rows.length > 0) {
                                        ...
                                        for (var index = 0; index < len; index++) {
                                            let objM: TopPlanM  = result.rows.item(index);
                                            objMs.push(objM);
                                        }
                ...
                () => {
                    this.db.create(ConectionDBName.connectionDB)
                        ...
                        (connection: SQLiteObject) => {
                            ...
                            `(
                                plancode TEXT, region TEXT, total TEXT,
                                PRIMARY KEY (plancode, region)
                                );`;
