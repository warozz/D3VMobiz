  ...
  "info": {
    ...
    "section2": {
      ...
      "data": [{
        ...
        "type": "image",
        "value": "assets/img/projectform/graph/02/graph_EN12.png",
  ...
  "summary": [
    ...
    },
    {
      ...
      "items": [
        ...
        "ควรศึกษาข้อมูลก่อนการตัดสินใจ อีกทั้งต้องการแถลงข้อมูลตามความเป็นจริงในเอกสารขอเอาประกัน เพื่อผลประโยชน์สูงสุดของท่าน \"เอกสารนี้เป็นเพียงการสรุปผลประโยชน์โดยสังเขป และขอสงวนสิทธิในการพิจารณารับประกันตามหลักเกณฑ์ของบริษัทฯ ทั้งนี้เงื่อนไขและความคุ้มครองอย่างสมบูรณ์จะถูกระบุอยู่ในกรมธรรม์ที่ท่านซื้อไว้เท่านั้น\""
      ]
    }
  ],
  "calculate": (sum, prospect, utils) => {
    const maxAge : number = 84;
    const payAge = Math.round(maxAge- utils.formatInt(prospect.age));

    const val0 = utils.numFormat(utils.formatInt(sum));
    const val1 = utils.numFormat(Math.round(utils.formatInt(sum)*0.01));
    const val2 = utils.numFormat(Math.round(utils.formatInt(sum)*1.1));
    const val3 = utils.numFormat(Math.round(utils.formatInt(sum)* utils.formatInt(payAge) /100));
    const val4 = utils.numFormat(Math.round(utils.formatInt(sum)*1.1));
    const val5 = utils.numFormat(Math.round(utils.formatInt(sum)* utils.formatInt(payAge) /100)  + Math.round(utils.formatInt(sum)*1.1)  );
    const val6 = utils.numFormat(Math.round(payAge));
    const val7 = utils.numFormat(Math.round(payAge + 110 ));
    const val8 = "ตั้งแต่วันครบรอบปีกรมธรรม์ที่ 1 ถึงอายุ "+maxAge+" ปี รวม  "+Math.round(payAge)+"   ครั้ง";
