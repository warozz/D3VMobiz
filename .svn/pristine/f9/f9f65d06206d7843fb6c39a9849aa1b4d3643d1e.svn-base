export default {
      ...
      "items": [
        ...
        },
        {
          ...
          subItems: [
            ...
            {
              name: "รวมรับเงินบำนาญรายเดือนตลอดสัญญาสูงสุด 372 งวด",
              percentIncome: "375.72%*",
              income: "{{income4}}"
            }
          ]
        }
    ...
    },
    {
      ...
      "items": [
        ...
        },
        {
          "iconText": "***",
          "color": "c-blue",
          "message": "ผู้เอาประกันภัยสามารถเลือกรับเงินบำนาญเป็นรายปี ปีละ {{income1}} บาท {{extendMessage2}} ตั้งแต่วันครบรอบปีกรมธรรม์ที่ผู้เอาประกันภัยอายุครบ 60 ปี ไปจนถึงวันครบรอบปีกรมธรรม์ที่อายุครบ 90 ปี"
        }
      ]
    },
    {
  ...
  "calculate": (sum, prospect, utils) => {
    const val0 = sum;
    const val1 = Math.round( utils.formatInt( sum ) * 0.12 ); // คืนเงินปีละ 12%  ของทุนประกัน ตั้งแต่อายุ 60 ถึง 90
    const val2 = Math.round( utils.formatInt( sum ) * 3.72 ); // คืนเงิน 12% เป็นเวลา 31 ปี = 372% ของทุนประกัน
    const val3 = Math.round( utils.formatInt( sum ) * 0.0101 ); // คืนเงินเดือนละ 1.01%  ของทุนประกัน ตั้งแต่อายุ 60 ถึง 90
    const val4 = Math.round( utils.formatInt( sum ) * 3.7572 ); // คืนเงินเดือนละ 1.01%  เป็นเวลา 31 ปี = 375.72% ของทุนประกัน

    let moreThan5000 = val3 > 5000; // แสดงข้อมูลคืนเงินเดือนเมื่อ เบี้ยต่อเดือนมากกว่า 5000 บาท
    let extendMessage1 = "";
    let extendMessage2 = "";
    if (moreThan5000) {
