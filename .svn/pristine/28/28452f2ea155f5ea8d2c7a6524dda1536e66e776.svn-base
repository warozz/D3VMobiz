export default {
    "name": "ทรัพย์ปันผล1 15/15 (มีเงินปันผล)",
    "info": {
      "section1": null,
      "section2": {
        "css": {
        ...
        "data": [{
          ...
          "type": "image",
          "value": "assets/img/projectform/graph/02/02_graph44.png",
    ...
    "calculate": (sum, prospect, utils, quatation) => {
      ...
      const val2 = utils.numFormat(Math.round(Math.round(utils.formatInt(sum)*0.01)*13)); 			
      const val3 = utils.numFormat(Math.round(utils.formatInt(sum)*1.2));
      const val4 = utils.numFormat(Math.round(utils.formatInt(sum)*1.33));
