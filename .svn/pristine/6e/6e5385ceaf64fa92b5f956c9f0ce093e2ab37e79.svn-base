export default {
  "name": "ทรัพย์ทวี 400 (มีเงินปันผล)",
  "info": {
    "section1": null,
    "section2": {
      "css": {
      ...
      "data": [{
        ...
        "type": "image",
        "value": "assets/img/projectform/graph/02/02_graph13.png",
  ...
  "calculate": (sum, prospect, utils, quatation) => {
    const val0 = utils.numFormat(utils.formatInt(sum));											
		...
		const val3 = utils.numFormat(Math.round( utils.formatInt( sum ) * 2.90 ));							
		const val4 = utils.numFormat(Math.round( utils.formatInt( sum ) * 4.1 ));								
