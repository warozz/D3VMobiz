export default {
  ...
  "info": {
    ...
    "section2": {
      ...
      "data": [{
        ...
        "type": "image",
        "value": "assets/img/projectform/graph/02/02_graph4.png",
      ...
      "items": [
        {
          ...
          "subItems": [
            {
              "name": "ตั้งแต่วันครบรอบปีกรมธรรม์ที่ 1 - 14 รวม 14 ครั้ง",
              "percentIncome": "56%*",
              "income": "{{income1}}"
            }
          ]
  ...
  "calculate": (sum, prospect, utils, quatation) => {
    ...
    const val0 = utils.numFormat(Math.round( utils.formatInt( sum ) * 1.2 )); 			
 		const val1 = utils.numFormat(Math.round( utils.formatInt( sum ) * 0.04 )); 			
 		const val2 = utils.numFormat(Math.round( utils.formatInt( sum ) * 0.04 ) * 14); 
		const val3 = utils.numFormat(Math.round( utils.formatInt( sum ) * 1.24 )); 		
		const val4 = utils.numFormat(Math.round( utils.formatInt( sum ) * 1.76 )); 	
