export default {
  ...
  "calculate": (sum, prospect, utils) => {
    ...
    const val2 = Math.round(Math.round(utils.formatInt(sum)*0.02)*18);
    const val3 = Math.round(utils.formatInt(sum)*1.10);
    const val4 = Math.round(utils.formatInt(sum)*1.46);
