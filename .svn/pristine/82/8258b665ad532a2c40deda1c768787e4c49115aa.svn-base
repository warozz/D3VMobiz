export default {
  "name": "ทรัพย์มิ่งขวัญ (มีเงินปันผล)",
  "info": {
    "section1": null,
    "section2": {
      "css": {
      ...
      "data": [{
        ...
        "type": "image",
        "value": "assets/img/projectform/graph/01/01_graph24.png",
  ...
  "summary": [
    ...
    },
    {
      ...
      "items": [
        ...
        "เบี้ยประกันชีวิตสามารถนำไปหักค่าลดหย่อนภาษีเงินได้บุคคลธรรมดาตามกฎหมายตามจำนวนที่จ่ายจริง แต่ไม่่เกินปีละ 100,000 บาท กรณีเพศ ชาย อายุ 69-70 ปี (ตามประกาศอธิบดีกรมสรรพากรเกี่ยวกับภาษีเงินได้ฉบับที่ 172)",
  ...
  "calculate": (sum, prospect, utils) => {
    const val0 = utils.numFormat(Math.round(utils.formatInt(sum)*0.20));
    const val1 = utils.numFormat(Math.round(utils.formatInt(sum)*0.50));
    const val2 = utils.numFormat(utils.formatInt(sum));
    const val3 = utils.numFormat(utils.formatInt(sum));
    const val4 = utils.numFormat(Math.round(utils.formatInt(sum)*1.5));
    const val5 = utils.numFormat(Math.round(utils.formatInt(sum)*1.5));
    const val6 = utils.numFormat(Math.round(utils.formatInt(sum)*1.5));
