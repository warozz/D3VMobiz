export default {
    "name": "ทรัพย์ปันผล1 10/10 (มีเงินปันผล)",
    "info": {
      "section1": null,
      "section2": {
        "css": {
        ...
        "data": [{
          ...
          "type": "image",
          "value": "assets/img/projectform/graph/02/02_graph43.png",
    ...
    "calculate": (sum, prospect, utils, quatation) => {
      ...
      const val2 = utils.numFormat(Math.round(Math.round(utils.formatInt(sum)*0.01)*8)); 			
      const val3 = utils.numFormat(Math.round(utils.formatInt(sum)*1.20));
      const val4 = utils.numFormat(Math.round(utils.formatInt(sum)*1.28));
      const val5 = utils.numFormat(Math.round(Math.round(utils.formatInt(sum)*0.01)*8));
      const val6 = utils.numFormat(Math.round(utils.formatInt(sum)*1.20));
      const val7 = utils.numFormat(Math.round(utils.formatInt(sum)*1.28));
