export default {
  "name": "ทรัพย์ทวี 300 (มีเงินปันผล)",
  "info": {
    "section1": null,
    "section2": {
      "css": {
      ...
      "data": [{
        ...
        "type": "image",
        "value": "assets/img/projectform/graph/02/02_graph12.png",
  ...
  "calculate": (sum, prospect, utils, quatation) => {
    const val0 = utils.numFormat(utils.formatInt(sum));											
		const val1 = utils.numFormat(Math.round( utils.formatInt( sum ) * 0.3 ));					
		const val2 = utils.numFormat(Math.round( utils.formatInt( sum ) *0.3)*4);
		const val3 = utils.numFormat(Math.round( utils.formatInt( sum ) * 1.90 ));							
		const val4 = utils.numFormat(Math.round( utils.formatInt( sum ) * 3.1 ));								
