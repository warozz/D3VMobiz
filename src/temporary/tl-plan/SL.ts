export default {
  "name": "เกษมทรัพย์ 130 (มีเงินปันผล)",
  "info": {
    "section1": null,
    "section2": {
      "css": {
      ...
      "data": [{
        ...
        "type": "image",
        "value": "assets/img/projectform/graph/02/02_graph3.png",
      ...
      "items": [
        {
          ...
          "subItems": [
            {
              "name": "ทุกวันครบรอบ 4 ปี กรมธรรม์ตั้งแต่ปีกรมธรรม์ที่ 4 - 16 รวม 4 ครั้ง ",
              "percentIncome": "16%*",
              "income": "{{income1}}"
            }
          ]
  ...
  "calculate": (sum, prospect, utils, quatation) => {
    const val0 = utils.numFormat(utils.formatInt(sum));											
		const val1 = utils.numFormat(Math.round( utils.formatInt( sum ) * 0.04 )); // คืนครั้งละ 4% ของทุนประกัน ทุกๆ 4 ปี
		const val2 = utils.numFormat(Math.round( utils.formatInt( sum ) * 0.04 ) * 4); // คืน 4% ของทุนประกัน 4 ครั้ง = 16%
		const val3 = utils.numFormat(Math.round( utils.formatInt( sum ) * 1.14 )); // คืนครั้งละ 100% + 10% + 4% ของทุนประกัน
		const val4 = utils.numFormat(Math.round( utils.formatInt( sum ) * 1.30 )); // คืนครั้งละ 100% + 10% + 4% + 16% ของทุนประกัน = 130%
