export default {
  ...
  "info": {
    ...
    "section2": {
      ...
      "data": [{
        ...
        "type": "image",
        "value": "assets/img/projectform/graph/02/02_graph58.png",
      ...
      "items": [
        {
          ...
          "subItems": [
            {
              "name": "ตั้งแต่วันครบรอบปีกรมธรรม์ที่ 1-6 รวม 6 ครั้ง",
              "percentIncome": "15%*",
              "income": "{{income1}}"
            }
          ]
  ...
  "calculate": (sum, prospect, utils) => {
    ...
    const val1 = utils.numFormat(Math.round(utils.formatInt(sum)*0.025));	
    const val2 = utils.numFormat(Math.round(Math.round(utils.formatInt(sum)*0.025)*6));
    const val3 = utils.numFormat(Math.round(utils.formatInt(sum)*1.0));
    const val4 = utils.numFormat(Math.round(utils.formatInt(sum)*1.15));
    const val5 = utils.numFormat((Math.round(utils.formatInt(sum)*1.0))+(Math.round(utils.formatInt(sum)*0.025)));
