export default {
  "name": "คุ้มทวี 5 เท่า 15/15 (มีเงินปันผล)",
  "info": {
    "section1": null,
    "section2": {
      "css": {
      ...
      "data": [{
        ...
        "type": "image",
        "value": "assets/img/projectform/graph/01/01_graph8.png",
  ...
  "calculate": (sum, prospect, utils, quatation) => {
    const val0 = utils.numFormat(utils.formatInt( sum )  * 5);											
		const val1 = utils.numFormat(Math.round( utils.formatInt( sum )  * 1.1 ));				
		const val2 = utils.numFormat(Math.round( utils.formatInt( sum )  * 5  ));
