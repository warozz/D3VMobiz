export default {
      ...
      "items": [
        {
          ...
          "subItems": [
            {
              "name": "ตั้งแต่วันครบรอบปีกรมธรรม์ที่ 2-59 รวม {{round1}} ครั้ง",
  ...
  "calculate": (sum, prospect, utils) => {
    const year = 60 - (utils.formatInt(prospect.age)+2);
    const year59 = Math.round(Math.round(utils.formatInt(sum)*0.02)*utils.formatInt(year));
    const Get110 = Math.round(utils.formatInt(sum)*1.1);

    const val0 = utils.formatInt(sum);
    const val1 = Math.round(utils.formatInt(sum)*0.02);
    const val2 = Math.round(Math.round(utils.formatInt(sum)*0.02)*utils.formatInt(year));
    const val3 = Math.round(utils.formatInt(sum)*1.1);
    const val4 = (Math.round(utils.formatInt(year)*2)+110);
    const val5 = utils.formatInt(year59)+utils.formatInt(Get110);
    const val7 = Math.round(utils.formatInt(year)*2);
    const val9 = utils.formatInt(year);
    const val11 = (Math.round(utils.formatInt(year)*2)+110);

    return {
      data2_1: utils.numFormat(val0),
      ...
      data3_7: utils.numFormat(val1),
      data3_8: utils.numFormat(val3),
      round1: utils.numFormat(val9),
      percent1: utils.numFormat(val7),
      income1: utils.numFormat(val2),
      income2: utils.numFormat(val3),
      income3: utils.numFormat(val5),
      percent2: utils.numFormat(val4)
    }
  }
}
