export default {
  ...
  "info": {
    ...
    "section2": {
      ...
      "data": [{
        ...
        "type": "image",
        "value": "assets/img/projectform/01_graph.png",
  ...
  "calculate": (sum, prospect, utils, quatation) => {
    ...
    const val0 = utils.numFormat(sum) ;											
 		const val1 = utils.numFormat(Math.round( utils.formatInt( sum ) * 1.2 )) ; // คืน 12 % ของทุนประกัน			
 		const val2 = utils.numFormat(Math.round( utils.formatInt( sum ) * 1.2 )) ; 			
		const val3 = utils.numFormat(Math.round( utils.formatInt( sum ) * 1.2 )) ;						//nfNL[1].setText(M.multiply(mPP.SUM,"2",0));
