...
/**
 * Generated class for the QuatationSignaturePage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-quatation-pdf',
  templateUrl: 'quatation-pdf.html',
})
export class QuatationPdfPage {
  ...
  constructor(
    ...
    console.log('isUlink-*--', this.appData.isUlink())
    // เชคว่าเป็น ยูลิงค์
    this.isUlink = this.appData.isUlink();
    ...
    if (typeof this.navParams.data.fromAppDetail != 'undefined') {
      ...
      this.appData.getData('mcaapplicationM').then((res: MCAapplicationsM) => {
        ...
        // ถ้าเป็น U-link ให้เปิดปุ่ม โหลด พิมพ์ ส่งเมล์
        if (this.isUlink) {
          this.fromAppDetail = false;
        }
  ...
  async getLoginProfile(){
    ...
    await this.storage.get('loginProfile').then(async profile => {
      ...
      if(profile.roleType == 'employee'){
        ...
        this.genQuoModel.agentFullName = profile.fName + ' ' + profile.lName;
        this.genQuoModel.agentBranch = profile.branch;
      }
    });
  }
        ...
        this.commonUtilProvider.callRestServiceTLPrompt(reqModel).then(
          (res: any) => {
            ...
            if (res && res.datas[0] && res.datas[0].binaryQuotationPDF) {
              ...
              this.updateRefno(res.datas[0].referenceNo, this.pdfName+this.pdfLanguage);  
  ...
  public updateRefno(refno, pdfpath) {
    this.storage.get('tlpromptMode').then(async mode => {
      if (0 == mode) {
                ...
                this.apiProvider.callData(reqM).then(
                  async (res) => {
                      ...
                      this.navCtrl.pop().then((res) => {
                        ...
                        this.navCtrl.setRoot(QuatationPdfPage, data).then(() => {
                          this.loading.dismiss();
                        });
