...
export class GroupPlanService {
                    ...
                    if (objMs != null && objMs.length > 0) {
                        ...
                        this.db.create(ConectionDBName.connectionDB).then((db: any) => {
                            ...
                            this.sqlitePorter.importJsonToDb(dbInstance, sqlJsonBlock).then(
                                ...
                                (err) => {
                                    console.log("******************* groupplan " + JSON.stringify(err));
                                    reject(err);
                                }
                            );
                          });
    ...
    public delete(params: any) {
        ...
        return new Promise((resolve, reject) => {
            this.createTable().then(
                (res) => {
                    let objMs: Array<GroupPlanM> = params; 
                    let paramSQL = [];
                                ...
                                (result) => {
                                    ...
                                    if (result.rows.length > 0) {
                                        ...
                                        for (var index = 0; index < len; index++) {
                                            let objM: GroupPlanM  = result.rows.item(index);
                                            objMs.push(objM);
                                        }
